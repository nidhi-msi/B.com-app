...
import { BookOpen, Search, Award, FileText, CheckSquare, Sparkles, GraduationCap, Home, Palette, Sun, Moon, X } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 glass-panel border-b backdrop-blur-md transition-colors duration-300" style={{ backgroundColor: 'var(--bg-nav)', borderColor: 'var(--border-color)' }}>
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 gap-2 sm:gap-4">
          ...
          >
            <div 
              className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl p-0.5 shadow-md group-hover:scale-105 transition-transform shrink-0"
            ...
            >
              <div className="w-full h-full rounded-[10px] flex items-center justify-center" style={{ backgroundColor: 'var(--bg-app)' }}>
                <GraduationCap className="w-4 h-4 sm:w-6 sm:h-6" style={{ color: 'var(--text-accent)' }} />
              </div>
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-extrabold text-base sm:text-xl tracking-tight font-serif whitespace-nowrap" style={{ color: 'var(--text-main)' }}>
                  IGNOU <span className="gradient-text">B.Com</span>
                </span>
                <span 
                  className="text-[8px] sm:text-[10px] font-bold tracking-widest px-1 sm:px-1.5 py-0.2 rounded border"
                  style={{ backgroundColor: 'var(--primary-light)', color: 'var(--text-accent)', borderColor: 'var(--border-color)' }}
                >
                  CBCS
                </span>
              </div>
              <p className="text-[10px] sm:text-[11px] hidden lg:block" style={{ color: 'var(--text-muted)' }}>Semester Hub & Exam Master</p>
            </div>
          </div>

          {/* Desktop & Tablet Search Bar */}
          <div className="hidden sm:block flex-1 max-w-xs md:max-w-md mx-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-1.5 sm:py-2 text-xs sm:text-sm rounded-xl border focus:outline-none focus:ring-1 transition-all"
                style={{ 
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs hover:opacity-100 opacity-70 p-0.5"
          ...
          {/* Right Controls: Mobile Search Toggle + Theme & Mode Switcher */}
          <div className="flex items-center gap-1.5 shrink-0">
            
            {/* Mobile Search Button (<640px) */}
            <button
              onClick={() => setShowMobileSearch(!showMobileSearch)}
              className="sm:hidden p-2 rounded-xl border text-xs font-semibold transition-all flex items-center justify-center"
              style={{ 
                backgroundColor: showMobileSearch ? 'var(--primary-light)' : 'var(--bg-card)', 
                borderColor: showMobileSearch ? 'var(--border-hover)' : 'var(--border-color)',
                color: showMobileSearch ? 'var(--text-accent)' : 'var(--text-main)'
              }}
              title="Toggle Search"
              aria-label="Search"
            >
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2 sm:px-2.5 py-1.5 rounded-xl border text-xs font-semibold transition-all hover:scale-105"
                style={{ 
              ...
              >
                ...
                <span className="hidden md:inline font-bold text-xs capitalize">{currentThemeObj.name}</span>
              ...
              {showThemeMenu && (
                <div 
                  className="absolute right-0 mt-2 w-56 sm:w-60 max-w-[calc(100vw-24px)] rounded-2xl p-2 shadow-2xl border backdrop-blur-xl z-50 animate-fadeIn"
                  style={{ 
        ...
        {/* Mobile Expandable Search Bar (When Search is clicked on mobile) */}
        {showMobileSearch && (
          <div className="sm:hidden pb-3 animate-fadeIn">
            <div className="relative">
              ...
              <input
                ...
                autoFocus
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-9 pr-14 py-2 text-xs rounded-xl border focus:outline-none focus:ring-1"
                style={{ 
                  backgroundColor: 'var(--bg-input)', 
                  borderColor: 'var(--border-color)', 
                  color: 'var(--text-main)'
              ...
              <button
                ...
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[11px] font-bold px-1.5 py-0.5 rounded opacity-80 hover:opacity-100"
                style={{ color: 'var(--text-muted)' }}
              >
                Close
              </button>
